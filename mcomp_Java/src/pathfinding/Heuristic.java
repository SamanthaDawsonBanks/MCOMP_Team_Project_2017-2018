...
/**
 * Class which contains all methods for returning the hx (Heuristic cost) between
 * two Vertices. Each method must take in the current Vertex and destination Vertex
 * to work out the estimated cost between these two points. Each method handles a
 * different implementation of calculating the hx value and is called in via the
 * AStar search (current algorithm).
 *  * 
...
public class Heuristic {
  ...
  /**
   * Method for working out the Manhattan distance between two vertices a and b.
  ...
  /**
   * Method for working out the Diagonal distance between two vertices a and b.
   * Find non negative difference in x values and y values between nodes.
   * 
   * @param Vertex a the current Vertex
   * @param Vertex b the destination Vertex
  ...
  /**
   * Method for working out the Euclidian distance between two vertices a and b.
   * Find non negative difference in x values and y values between nodes.
   * 
   * @param Vertex a the current Vertex
   * @param Vertex b the destination Vertex
   ...
   * 
   * It calculates distance similar to the Manhattan heuristic. However during runtime
   * it checks the direction the robot is facing, and the direction it would be after it 
   * moved to the child Vertex.
   * 
   * If the current facing direction equals the neighbours facing direction we score that Vertex better.
   * So if the "robot" is facing north at (2,8), and its children are (3,8) and (2,9).
   * (3,8) would allow the robot to stay facing north and therefore scores this neighbour better.
   * 
   * 
