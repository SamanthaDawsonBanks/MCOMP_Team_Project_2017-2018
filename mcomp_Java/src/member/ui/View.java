...
public class View extends Application {
  ...
  private RemoteMember member = null;
  private Member parent;
  private Pane pane;
  private HBox hbox;
  private Label label;
  private int counter = 0;
  private Group circleGroup, rectangleGroup, lineGroup, amalgamateGroup, pathGroup, searchedGroup, optimizedGroup;
  ...
  public VBox getVBoxMap() {
    ...
    lidarBtn.setOnAction(event ->{
      if(lidarBtn.isSelected()) {
        ...
        try {
          pane.getChildren().add(drawCircle(member.getLocalHerdData().getMap().getLayer(spinner.getValue()).getWaypoints()));
    ...
    mapBtn .setOnAction(event ->{
      if(mapBtn.isSelected()) {
        ...
        try {
          pane.getChildren().add(drawAmalgamatedMap(member.getLocalHerdData().getMap().getAmalgamatedMap()));
        } catch (RemoteException e) {
          // TODO Auto-generated catch block
          e.printStackTrace();
        }
